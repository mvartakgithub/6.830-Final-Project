...
\section{Introduction}
	...
	To address this issue, we designed PubCrawl, a paper search engine. Using this system allows researchers to query for the most important and relevant papers in a field, particularly one they are not familiar with. PubCrawl uses Apache Lucene for text indexing to measure a paper’s relevance to a query, and an adaptation of PageRank on the papers citation graph for weighting a paper’s importance. Currently all papers in the Association of Computing Machinery (ACM) Digital Library database have been processed. A simple web user interface is provided, where users can input queries and weights for what attributes they feel is most important, to tune the result rankings to fit what they feel is most appropriate. The top papers are returned, with hyperlinks to the papers’ ACM homepage.
...
There are a few paper search engines currently in existence: CiteseerX, Google Scholar, and Microsoft Academic Search.
...
\begin{itemize}
\item For the score source $E$, we use a uniform value. We minimize the value of $E$ that still results in convergence, to reduce the total amount of freely created score. 
