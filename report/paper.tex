...
Library available at \url{http://saw.csail.mit.edu:9000}. The system allows
users to weight the relative importance of matches in various sections of the
documents. We also implement a version of PageRank over the dataset. We
evaluate the system in user studies and find it produces comparable results to
...
Here, we present PubCrawl, a new search engine for the ACM Digital Library
available at \url{http://saw.csail.mit.edu:9000}. The PubCrawl interface allows
users to submit search terms and assign relative weights to title, abstract,
and full text searches and PageRank. It uses the Django web framework for the
front-end interface and is backed by a Postgres database which stores metadata
(authors, venues, and citations) and a Lucene search index. Figure
\ref{interface} shows a screenshot of the interface.
